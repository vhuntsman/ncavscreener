    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            foreach (var quote in stockArray)
            {
                ...
                var ncav = parser.GetNcav(balanceSheetResponse.Content, ticker);
                var price = double.Parse(quote["regularMarketPrice"]["fmt"].Value);
                string longName = string.Empty;
                try
                {
                    longName = quote["longName"].Value;
                }
                catch
                {
                ...
                if (!double.IsNaN(ncav) && (ncav > 0.0d))
                {
                    if (price < ncav)
                    {
                        Console.WriteLine(String.Format("{0,5} | {1, 8} | {2, 8} | {3, 55} | {4, 20}", ticker, price, ncav, longName, fullExchangeName));
