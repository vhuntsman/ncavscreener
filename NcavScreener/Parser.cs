...
namespace NcavScreener
{
    public class Parser
    {
        ...
        {
            // Pattern for crumb
            Regex re = new Regex(@"""user"":{""crumb"":""(.*?)"".*?}", RegexOptions.Compiled | RegexOptions.IgnoreCase | RegexOptions.Singleline);
            var matches = re.Matches(context);
            return Regex.Unescape(matches[0].Groups[1].Value);
        }

        public double GetNcav(string context, string ticker, double sharesOutstanding)
        {
            ...
            try
            {
                ...
                result = (currentAssets - totalLiabilities) / (sharesOutstanding / 1000);
